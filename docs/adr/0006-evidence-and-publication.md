...
3. **Taito is approved, with a fixed attribution string.** 台東区's open-data terms (CC BY 4.0) were verified against the publisher's own pages on 2026-09-20 and its display example identifies four elements. `TAITO_ATTRIBUTION_TEXT` contains all four verbatim — `台東区`, `CC-BY表示4.0国際`, `本作品の内容について、台東区は一切保証しないものとする。` and the original-data URL — and nothing else: no additional restriction is invented. The **stable dataset page** is the original-data URL, because the release CSV URL changes with every 時点 release and is release provenance rather than a citation. The string is a single constant, so the tile and spot-detail endpoints send byte-identical `sources[].attributionText` (both build `sources` from the same registry row through the same DTO; tested on both endpoints).
